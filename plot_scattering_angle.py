...
def plot_data(data):
    ...
    input_data = [i[:2] for i in data["results"]]
    ...
    axpx2 = axpx.twinx()
    h2, = axpx2.plot(angles, avg_butmodi[:,0], color="C3", label=r"With modus removed")
    axpx2.fill_between(angles, avg_butmodi[:,0] + stddev_modi[:,0], avg_butmodi[:,0] - stddev_modi[:,0], color="C3")
    ...
    h1, = axpx.plot(angles, modi[:,0], ".", color="C1", label=r"modus$(p_x)$")
    axpx.grid()
    axpx.set_title("$p_x$")
    ...
    axpy2 = axpy.twinx()
    h2, = axpy2.plot(angles, avg_butmodi[:,1], color="C4", label=r"With modus removed")
    axpy2.fill_between(angles, avg_butmodi[:,1] + stddev_modi[:,1], avg_butmodi[:,1] - stddev_modi[:,1], color="C4")
    ...
    h1, = axpy.plot(angles, modi[:,1], ".", color="C2", label=r"modus$(p_y)$")
    axpy.grid()
    axpy.set_title("$p_y$")
